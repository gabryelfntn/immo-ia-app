...
function DetailRow({
  ...
  return (
    <div className="border-b border-white/[0.06] py-4 last:border-0">
      ...
      <dd className="mt-1.5 text-sm font-medium text-zinc-50">{children}</dd>
    </div>
  );
}
...
function PulsingStatusBadge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-2 rounded-full border border-white/[0.08] px-3 py-1.5 text-xs font-semibold shadow-lg backdrop-blur-md ${statusBadgeClass(status)}`}
    >
      <span
...
export default async function PropertyDetailPage({ params }: Props) {
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      <div className="mt-8 flex flex-col gap-4 rounded-2xl border border-white/[0.08] bg-white/[0.03] p-5 backdrop-blur-sm sm:flex-row sm:items-end sm:justify-between">

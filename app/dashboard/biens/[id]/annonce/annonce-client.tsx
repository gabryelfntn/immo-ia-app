...
function CopyTextButton({ text, label }: { text: string; label: string }) {
  ...
  return (
    <button
      type="button"
      onClick={onCopy}
      className="rounded-lg border border-white/10 bg-[#0a0a0f] px-3 py-1.5 text-xs font-semibold text-indigo-200 transition-all duration-300 hover:border-indigo-500/40 hover:bg-indigo-500/10"
    >
      {done ? "Copié !" : label}
    </button>
  );
  ...
  return (
    <div className="space-y-10">
      <section className="rounded-2xl border border-white/[0.08] bg-[#12121a] p-6 card-luxury sm:p-8">
        ...
        <p className="mt-4 text-xl font-bold text-white">{header.title}</p>
      ...
      <div className="relative">
        {loading ? (
          <div
            className="pointer-events-none absolute inset-0 z-10 flex items-start justify-center rounded-2xl bg-[#0a0a0f]/85 pt-24 backdrop-blur-sm"
            aria-live="polite"
            aria-busy="true"
          >
            <div className="flex flex-col items-center gap-3 rounded-2xl border border-white/10 bg-[#12121a] px-8 py-6 shadow-2xl">
              <span
        ...
        <div className={loading ? "min-h-[12rem] opacity-50" : ""}>
          <div className="flex flex-col gap-4 rounded-2xl border border-white/[0.08] bg-[#12121a]/80 p-6 sm:flex-row sm:items-center sm:justify-between">
      ...
      <section>
        <h2 className="text-2xl font-bold text-white">Annonces générées</h2>
        <p className="mt-2 text-sm text-zinc-500">
          Les générations les plus récentes apparaissent en premier.
        </p>

        {listings.length === 0 ? (
          <p className="mt-8 rounded-2xl border border-dashed border-white/10 bg-[#12121a]/50 px-6 py-12 text-center text-sm text-zinc-500">
